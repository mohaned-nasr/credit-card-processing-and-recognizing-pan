...
def crop_pan(warped_img):
    # Assuming typical PAN location relative to the warped image
    h, w = warped_img.shape[:2]

    # These values (0.5, 0.7, 0.1, 0.9) are just an example and should be adjusted based on your PAN location
    pan_y_start_ratio = 0.5  # Start 50% down from the top
    pan_y_end_ratio = 0.7  # End 70% down from the top
    pan_x_start_ratio = 0.1  # Start 10% from the left
    pan_x_end_ratio = 0.9  # End 90% from the left
...
def pre_process(img):
    ...
    # Ensure that the straighten function has detected a card
    if img_straightened is None:
        print("No card detected.")
        return None

    img_median = cv2.medianBlur(img_straightened, 5)
    ...
    cv2.imshow('Processed Image', img_opened)

    img_resized = cv2.resize(img_opened, (1000, 700))
    # cv2.imshow('resized Image', img_resized)

    # Crop the PAN from the straightened image
