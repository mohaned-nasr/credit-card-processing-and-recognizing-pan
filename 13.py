...
def straighten(img, colored):
    ...
    if corners is not None:
        ...
        sharpened_warped_image = sharpen_image(warped_image)

        h, w = sharpened_warped_image.shape[:2]
...
def pre_process(img):
    ...
    #otsu_threshold_value, img_thresh = cv2.threshold(img_straightened, 150, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    _, img_thresh = cv2.threshold(img_straightened, 95, 255, cv2.THRESH_BINARY)
...
# Example usage:
input_img = cv2.imread('CSE483 SMR24 Project Test Cases/13 - Matozbot el camera ya Kimo.jpg')
